...
################################################################################
#	The following functions all correspond to facebook
################################################################################
...
def graph():
    ...
    response = make_response(render_template("graph.html",ACTION = upload_url))
    return response
...
def fb_publish(blob_key, message):
    ...
    try:
        ...
        if blob_key is not None:
            ...
            # create the url from which the image can be retrieved
            image_url = request.url_root + 'img/' + blob_key
            response = make_response(render_template("postpic.html", URL = image_url, MSG = message))
            return response
